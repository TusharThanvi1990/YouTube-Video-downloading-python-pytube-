...
def download_video(link,directory):
    ...
    command = int(input())
    if command == 1:
        print("Below are the all stream option to download (it may take few seconds )\n")
        print(ytobj.streams.filter())
    elif command == 2:
        print("Below are the stream option in mp4 format to download (it may take few seconds )\n")
        print(ytobj.streams.filter(file_extension = 'mp4'))
    elif command == 3:
        print("Below are the stream option in only audio format to download (it may take few seconds )\n")
        print(ytobj.streams.filter(only_audio=True))
    elif command == 4:
        print("Below are the stream option in only video format to download (it may take few seconds )\n")
